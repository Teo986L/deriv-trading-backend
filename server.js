...
function getTTLAlignedToCandle(timeframeSeconds) {
  ...
  return Math.max(secondsUntilClose - CANDLE_CLOSE_MARGIN, 3);
}
...
const ALL_TIMEFRAMES_CONFIG = {
  ...
  'H1':  { key: 'H1',  seconds: 3600,  candleCount: 80,  minRequired: 30 },
  ...
  const fetchPromise = (async () => {
    try {
      ...
      if (!Array.isArray(candles)) {
        console.warn(`⚠️ Resposta inesperada para ${tf.key}:`, candles);
        return candles;
      }
...
// ✅ ALTERAÇÃO 6 — Reconexão automática ao Deriv quando o WebSocket cai
async function getDerivClient() {
  // Se cliente existe, verifica se o WebSocket ainda está aberto (readyState === 1 = OPEN)
  if (derivClient && derivConnectionPromise) {
    const wsReady = derivClient.ws && derivClient.ws.readyState === 1;
    if (wsReady) return derivClient;

    // WebSocket caiu — reset completo para reconectar
    console.warn('⚠️ Conexão Deriv inativa (WS fechado), reconectando...');
  ...
  derivConnectionPromise = derivClient.connect()
    .then(() => {
      console.log('✅ Cliente Deriv pronto (autorizado)');
      return derivClient;
    })
    .catch(err => {
      console.error('❌ Falha conexão Deriv:', err);
      derivConnectionPromise = null;
      derivClient = null; // reset para tentar de novo na próxima chamada
      throw err;
    });
...
// ── tick com timeout de 350ms — fallback imediato para preço de candle ────────
async function getCurrentPrice(client, symbol) {
  return new Promise((resolve) => {
    ...
    if (client.ws && client.ws.readyState === client.ws.OPEN) {
      client.ws.send(JSON.stringify({ ticks: symbol, subscribe: 1, req_id: reqId }));
    } else {
      clearTimeout(timeout);
      if (typeof client.removeListener === 'function') client.removeListener(reqId, handler);
      resolve(null);
...
app.post('/api/analyze', authenticateToken, analyzeLimiter, async (req, res) => {
  ...
  const totalTimer = `⏱️ TOTAL_${requestId}`;
  ...
  try {
    ...
    // ── Candles ajustados por tipo de ativo ──────────────────────────────────
    ...
    // ── Fetch candles + tick em paralelo, com timeout por TF ─────────────────
    console.time(`⏱️ Fetch_${requestId}`);
    ...
    await Promise.all(
      allTfKeys.map(async (tfKey) => {
        ...
        try {
          ...
          // Fallback: tenta usar cache stale se o erro foi timeout
    ...
    console.timeEnd(`⏱️ Fetch_${requestId}`);
    ...
    // ── Analisar TFs em paralelo ──────────────────────────────────────────────
    console.time(`⏱️ Analisar_${requestId}`);
    await Promise.all(
    ...
    console.timeEnd(`⏱️ Analisar_${requestId}`);

    // ── Consolidar sinais ─────────────────────────────────────────────────────
    console.time(`⏱️ Consolidate_${requestId}`);
    ...
    console.timeEnd(`⏱️ Consolidate_${requestId}`);
    ...
    if (tickResult) {
      ...
      for (const tf of ['M1','M5','M15','H1','H4','H24']) {
    ...
    console.time(`⏱️ Suggestion_${requestId}`);
    ...
    console.timeEnd(`⏱️ Suggestion_${requestId}`);

    const primarySignal = consolidated.simpleMajority.signal;

    // ── analiseRefinada em paralelo com timing e liquidez ─────────────────────
    const analiseRefinadaPromise = (async () => {
      console.time(`⏱️ TraderBot_${requestId}`);
      try {
        ...
        console.timeEnd(`⏱️ TraderBot_${requestId}`);
        return { analiseRefinada: analise, validacaoRisco: risco };
      } catch (err) {
        console.timeEnd(`⏱️ TraderBot_${requestId}`);
        console.error('❌ analiseRefinada:', err.message);
        return { analiseRefinada: { erro: err.message, sinal: { direcao: 'HOLD', confianca: 0 } }, validacaoRisco: null };
      }
    })();
    ...
    // ── Deteção de Liquidez ───────────────────────────────────────────────────
    console.time(`⏱️ Liquidity_${requestId}`);
    ...
    console.timeEnd(`⏱️ Liquidity_${requestId}`);
...
const server = app.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`📊 Candles ajustados por tipo de ativo`);
  console.log(`🔌 Reconexão automática ao Deriv activa`);
